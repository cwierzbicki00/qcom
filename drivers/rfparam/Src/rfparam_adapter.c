...
#if defined(RFPARAM_QCC743)
#include "qcc743_aon.h"
#include "qcc743_mfg_media.h"
#include "qcc743_hbn.h"
#endif

#if defined(RFPARAM_QCC74x_undef)
#include "qcc74x_undef_aon.h"
#include "qcc74x_undef_mfg_media.h"
#include "qcc74x_undef_hbn.h"
#endif
...
QCC74xLOG_DEFINE_TAG(RFPARAM, DBG_TAG, true);
...
void rfparam_set_capcode(uint8_t capcode_in, uint8_t capcode_out)
{
    ...
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, AON_XTAL_CAPCODE_IN_AON, capcode_in);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, AON_XTAL_CAPCODE_OUT_AON, capcode_out);
...
#define WL_API_RMEM_ADDR  0x20010600
    ...
    qcc74x_ef_ctrl_read_common_trim(NULL, "dcdc_trim", &trim, 1);
    if (trim.empty) {
    ...
    qcc74x_ef_ctrl_read_common_trim(NULL, "icx", &trim, 1);
    if (trim.empty) {
    ...
    qcc74x_ef_ctrl_read_common_trim(NULL, "iptat", &trim, 1);
    if (trim.empty) {
    ...
    qcc74x_ef_ctrl_read_common_trim(NULL, "tmp_mp2", &trim, 1);
    if (trim.en == 1 && trim.parity == qcc74x_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
        g_rfparam_cfg->param.pwrcal.Temperature_MP = trim.value;
        rfparam_printf("tmp_mp2 value %d\r\n",(int)trim.value);
    }else{
        qcc74x_ef_ctrl_read_common_trim(NULL, "tmp_mp1", &trim, 1);
        if (trim.en == 1 && trim.parity == qcc74x_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
            g_rfparam_cfg->param.pwrcal.Temperature_MP = trim.value;
            rfparam_printf("tmp_mp1 value %d\r\n",(int)trim.value);
        }else{
            qcc74x_ef_ctrl_read_common_trim(NULL, "tmp_mp0", &trim, 1);
            if (trim.en == 1 && trim.parity == qcc74x_ef_ctrl_get_trim_parity(trim.value, trim.len)) {
                g_rfparam_cfg->param.pwrcal.Temperature_MP = trim.value;
