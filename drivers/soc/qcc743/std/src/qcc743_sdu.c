...
{
    (pHidSdio_RegMap_t)(QCC74x_SDIO_CCR_BASE),
#if NUM_FUNC == 2
    (pHidSdio_RegMap_t)(QCC74x_SDIO_CCR_BASE+QCC74x_SDIO_CCR_FUNC_OFFSET)
#elif NUM_FUNC == 3
    (pHidSdio_RegMap_t)(QCC74x_SDIO_CCR_BASE+QCC74x_SDIO_CCR_FUNC_OFFSET),
    (pHidSdio_RegMap_t)(QCC74x_SDIO_CCR_BASE+(2 * QCC74x_SDIO_CCR_FUNC_OFFSET))
#endif
};
...
uint32_t sdio_ioctl(uint32_t port_id, SDIO_CMD_TYPE cmd, void *arg)
{
    ...
    case IOCTL_HID_GET_BLOCK_SIZE:
	{
        ...
        {
            uint32_t blockSize = QCC74x_REGS8(SDIO_FN1_BLK_SIZE_0);

            blockSize |= ((QCC74x_REGS8(SDIO_FN1_BLK_SIZE_1) & 
