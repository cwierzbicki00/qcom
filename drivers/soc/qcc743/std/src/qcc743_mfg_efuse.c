...
static int mfg_efuse_get_empty_slot(char *name, int *slot, int32_t *last_val, uint8_t reload)
{
    ...
    *last_val = -1;
    ...
    if (rf_cal_slots >= 1) {
        buf[len] = '0';
        qcc74x_ef_ctrl_read_common_trim(NULL, buf, &trim, reload);
        if (trim.empty) {
...
int mfg_efuse_write_xtal_capcode_pre(uint8_t capcode, uint8_t program)
{
    ...
    if (slot == -1) {
        ...
        qcc74x_ef_ctrl_write_common_trim(NULL, buf, capcode, program);
...
int mfg_efuse_write_hp_poweroffset_pre(int8_t pwr_offset[14], uint8_t program)
{
    ...
    if (slot == -1) {
        ...
        qcc74x_ef_ctrl_write_common_trim(NULL, buf, cur_val, program);
        mfg_print("Write slot:%d\r\n", slot);
        return 0;
    }
}
...
{
    qcc74x_ef_ctrl_write_direct(NULL, addr, pword, countInword, program);
}

void mfg_efuse_read(uint32_t addr, uint32_t *pword, uint32_t countInword, uint8_t reload)
{
    qcc74x_ef_ctrl_read_direct(NULL, addr, pword, countInword, reload);
}
