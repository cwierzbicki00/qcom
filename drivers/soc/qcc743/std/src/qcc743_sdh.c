...
void SDH_EnableDMA(SDH_DMA_Mode_Type dmaMode)
{
    ...
    /* Select DMA type */
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_HOST_CTRL);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_DMA_SEL, dmaMode);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_HOST_CTRL, tmpVal);

    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_TRANSFER_MODE);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_DMA_EN);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_TRANSFER_MODE, tmpVal);
}
...
void SDH_DisableDMA(void)
{
    ...
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_TRANSFER_MODE);
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_DMA_EN);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_TRANSFER_MODE, tmpVal);
}
/****************************************************************************/ /**
...
SDH_Stat_Type SDH_ConfigDataTranfer(SDH_Data_Cfg_Type *dataCfg)
{
    ...
    if (dataCfg != NULL) {
        ...
        if (dataCfg->dataType == SDH_TRANS_DATA_BOOT_CONT) {
            tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_BLOCK_GAP_CTRL);
            /* Clear stop at block gap request */
            tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_STOP_AT_BLOCK_GAP_REQ);
            /* Continous transfer data */
            tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_CONT_REQ);
            QCC74x_WR_REG16(SDH_BASE, SDH_SD_BLOCK_GAP_CTRL, tmpVal);

            return SDH_STAT_SUCCESS;
        }

        tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_PRESENT_STATE_1);

        /* Check data inhibit flag */
        if (QCC74x_IS_REG_BIT_SET(tmpVal, SDH_CMD_INHIBIT_DAT)) {
        ...
        if (dataCfg->rxData) {
            tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_TO_HOST_DIR);
        } else {
            tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_TO_HOST_DIR);
        }

        tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_AUTO_CMD_EN, 0x00);

        /* auto command 23, auto send set block count cmd before multiple read/write */
        if (dataCfg->enableAutoCommand23) {
            tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_AUTO_CMD_EN, 0x02);
            QCC74x_WR_REG(SDH_BASE, SDH_SD_SYS_ADDR_LOW, dataCfg->blockCount);
        } else if (dataCfg->enableAutoCommand12) {
            /* auto command 12 */
            tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_AUTO_CMD_EN, 0x01);
        }

        if (dataCfg->blockCount > 1U) {
            tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_MULTI_BLK_SEL);
        } else {
            tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_MULTI_BLK_SEL);
        }

        /* Enable block count */
        tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_BLK_CNT_EN);
        ...
        if (dataCfg->dataType != SDH_TRANS_DATA_BOOT) {
            ...
            QCC74x_WR_REG16(SDH_BASE, SDH_SD_BLOCK_SIZE, dataCfg->blockSize);
            QCC74x_WR_REG16(SDH_BASE, SDH_SD_BLOCK_COUNT, dataCfg->blockCount);
        }
    } else {
        /* clear data flags */
        tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_TRANSFER_MODE);
        ...
        /* Check data inhibit flag */
        if (QCC74x_IS_REG_BIT_SET(tmpVal, SDH_CMD_INHIBIT_CMD)) {
            return SDH_STAT_BUSY;
        }
    }
...
SDH_Stat_Type SDH_GetCmdResp(SDH_CMD_Cfg_Type *cmd)
{
    if (cmd->respType != SDH_RESP_NONE) {
        cmd->response[0U] = QCC74x_RD_REG(SDH_BASE, SDH_SD_RESP_0);
        cmd->response[1U] = QCC74x_RD_REG(SDH_BASE, SDH_SD_RESP_2);
        cmd->response[2U] = QCC74x_RD_REG(SDH_BASE, SDH_SD_RESP_4);
        cmd->response[3U] = QCC74x_RD_REG(SDH_BASE, SDH_SD_RESP_6);
    }
...
uint32_t SDH_ReadDataPort(SDH_Data_Cfg_Type *dataCfg)
{
    ...
    tmpVal = QCC74x_RD_REG(SDH_BASE, SDH_SD_TRANSFER_MODE);

    if (!QCC74x_IS_REG_BIT_SET(tmpVal, SDH_DMA_EN)) {
        do {
            dataCfg->rxData[dataCfg->rxDataLen++] = QCC74x_RD_REG(SDH_BASE, SDH_SD_BUFFER_DATA_PORT_0);
            recvLen += 4;
...
uint32_t SDH_WriteDataPort(SDH_Data_Cfg_Type *dataCfg)
{
    ...
    if (!QCC74x_IS_REG_BIT_SET(tmpVal, SDH_DMA_EN)) {
        QCC74x_WR_REG(SDH_BASE, SDH_SD_BUFFER_DATA_PORT_0, dataCfg->txData[dataCfg->txDataLen++]);
    }
...
void SDH_SendCommand(SDH_CMD_Cfg_Type *cmd)
{
    ...
    tmpVal = QCC74x_RD_REG(SDH_BASE, SDH_SD_PRESENT_STATE_1);
    ...
    if (((tmpVal & SDH_PRESENT_FLAG_CMD_INHBIT) == 0U) && (cmd->type != SDH_CMD_EMPTY)) {
        ...
        tmpVal = flag >> 16;
        tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_CMD_TYPE, cmd->type);
        tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_CMD_INDEX, cmd->index);

        /* Config command argument */
        QCC74x_WR_REG(SDH_BASE, SDH_SD_ARG_LOW, cmd->argument);

        /* Config command index */
        QCC74x_WR_REG16(SDH_BASE, SDH_SD_CMD, tmpVal);
    }
...
void SDH_SetSdClock(uint32_t srcClock, uint32_t busClock)
{
    ...
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_CLOCK_CTRL);

    /* Disable internal clock*/
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_INT_CLK_EN);
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_SD_CLK_EN);

    /* Set divdier */
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_SD_FREQ_SEL_LO, div & 0xff);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_SD_FREQ_SEL_HI, (div >> 8) & 0x3);
    /* 0x1 = Programmable Clock Mode,0x0 = Divided Clock mode */
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_CLK_GEN_SEL, 0);

    /* Enable internal clock*/
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_INT_CLK_EN);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_SD_CLK_EN);

    QCC74x_WR_REG16(SDH_BASE, SDH_SD_CLOCK_CTRL, tmpVal);

    /* Wait for clock stable */
    do {
        tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_CLOCK_CTRL);
    } while (!QCC74x_IS_REG_BIT_SET(tmpVal, SDH_INT_CLK_STABLE));
}
...
void SDH_DisableSdClock(void)
{
    ...
    tmpVal = QCC74x_RD_REG(SDH_BASE, SDH_SD_CLOCK_CTRL);

    /* Disable internal clock*/
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_INT_CLK_EN);
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_SD_CLK_EN);

    QCC74x_WR_REG(SDH_BASE, SDH_SD_CLOCK_CTRL, tmpVal);
}
/****************************************************************************
...
void SDH_Powon(void)
{
    uint32_t tmpVal;
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_HOST_CTRL);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_SD_BUS_POWER);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_HOST_CTRL, tmpVal);
}
/****************************************************************************
 * @brief  SDH init function
...
void SDH_Ctrl_Init(const SDH_Cfg_Type *cfg)
{
    ...
    if (cfg->dataWidth == SDH_DATA_BUS_WIDTH_8BITS) {
        tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_EX_DATA_WIDTH);
    } else {
        tmpVal = QCC74x_CLR_REG_BIT(tmpVal, SDH_EX_DATA_WIDTH);
        tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_DATA_WIDTH, cfg->dataWidth);
    }

    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_HI_SPEED_EN, cfg->highSpeed);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_SD_BUS_VLT, cfg->volt);

    QCC74x_WR_REG16(SDH_BASE, SDH_SD_HOST_CTRL, tmpVal);

    if (cfg->vlot18Enable) {
        tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_HOST_CTRL_2);
        tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_UHS_MODE_SEL, cfg->speed);
        tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_SDH_V18_EN, cfg->vlot18Enable);
        QCC74x_WR_REG16(SDH_BASE, SDH_HOST_CTRL_2, tmpVal);
    }

    tmpVal = QCC74x_RD_REG(SDH_BASE, SDH_TX_CFG_REG);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_TX_INT_CLK_SEL);
    QCC74x_WR_REG(SDH_BASE, SDH_TX_CFG_REG, tmpVal);
...
void SDH_Reset(void)
{
    ...
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_TIMEOUT_CTRL_SW_RESET);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, SDH_SW_RST_ALL);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_TIMEOUT_CTRL_SW_RESET, tmpVal);

    /* Wait for reset done */
    do {
        tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_TIMEOUT_CTRL_SW_RESET);
    } while (QCC74x_IS_REG_BIT_SET(tmpVal, SDH_SW_RST_ALL));
}
/****************************************************************************/ /**
...
void SDH_Set_Timeout(uint8_t tmo)
{
    ...
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_TIMEOUT_CTRL_SW_RESET);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_TIMEOUT_VALUE, tmo);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_TIMEOUT_CTRL_SW_RESET, tmpVal);
}
...
SDH_Stat_Type SDH_SetInternalDmaConfig(SDH_DMA_Cfg_Type *dmaCfg, const uint32_t *data, uint8_t enAutoCmd23)
{
    ...
    if (dmaCfg->dmaMode == SDH_DMA_MODE_SIMPLE) {
        ...
        if (enAutoCmd23) {
            QCC74x_WR_REG(SDH_BASE, SDH_SD_ADMA_SYS_ADDR_1, (uintptr_t)data);
        } else {
            QCC74x_WR_REG(SDH_BASE, SDH_SD_SYS_ADDR_LOW, (uintptr_t)data);
        }
    } else {
        /* When use ADMA, disable simple DMA */
        QCC74x_WR_REG(SDH_BASE, SDH_SD_SYS_ADDR_LOW, (uint32_t)0);
        QCC74x_WR_REG(SDH_BASE, SDH_SD_ADMA_SYS_ADDR_1, (uintptr_t)(dmaCfg->admaEntries));
    }
    ...
    tmpVal = QCC74x_RD_REG16(SDH_BASE, SDH_SD_CLOCK_AND_BURST_SIZE_SETUP);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_DMA_SIZE, dmaCfg->fifoThreshold);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, SDH_BRST_SIZE, dmaCfg->burstSize);
    QCC74x_WR_REG16(SDH_BASE, SDH_SD_CLOCK_AND_BURST_SIZE_SETUP, tmpVal);
...
SDH_Stat_Type SDH_TransferBlocking(SDH_DMA_Cfg_Type *dmaCfg, SDH_Trans_Cfg_Type *transfer)
{
    ...
    /*
	tmp = QCC74x_RD_REG16(SDH_BASE,SDH_SD_ADMA_ERROR_STATUS);
	qcc74x_platform_printf("SDH_SD_ADMA_ERROR_STATUS->0x%x!\r\n",tmp);
	tmp = QCC74x_RD_REG16(SDH_BASE,SDH_SD_AUTO_CMD12_ERROR_STATUS);
	*/
