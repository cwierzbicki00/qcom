 ...
 *
 *******************************************************************************/
static QCC74x_Err_Type ATTR_TCM_SECTION pm_clr_pds_gpio_int(void)
{
    uint32_t tmpVal = 0;
 ...
 *
 *******************************************************************************/
QCC74x_Err_Type ATTR_TCM_SECTION AON_Set_LDO11_SOC_Sstart_Delay(uint8_t delay)
{
    uint32_t tmpVal = 0;
    ...
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, AON_DCDC_SSTART_TIME_AON, delay);
...
QCC74x_Sts_Type pm_pds_get_wakeup_src(uint32_t WakeupType)
{
    ...
    tmpVal = QCC74x_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QCC74x_GET_REG_BITS_VAL(tmpVal, PDS_RO_PDS_WAKEUP_EVENT);
...
void pm_pds_mask_all_wakeup_src(void)
{
    ...
    tmpVal = QCC74x_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QCC74x_CLR_REG_BIT(tmpVal, PDS_CR_PDS_WAKEUP_SRC_EN);
    QCC74x_WR_REG(PDS_BASE, PDS_INT, tmpVal);

    tmpVal = QCC74x_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, PDS_CR_PDS_RF_DONE_INT_MASK);
    QCC74x_WR_REG(PDS_BASE, PDS_INT, tmpVal);

    tmpVal = QCC74x_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, PDS_CR_PDS_WIFI_TBTT_SLEEP_IRQ_MASK);
    QCC74x_WR_REG(PDS_BASE, PDS_INT, tmpVal);

    tmpVal = QCC74x_RD_REG(PDS_BASE, PDS_INT);
    tmpVal = QCC74x_SET_REG_BIT(tmpVal, PDS_CR_PDS_WIFI_TBTT_WAKEUP_IRQ_MASK);
    QCC74x_WR_REG(PDS_BASE, PDS_INT, tmpVal);
}
                                        ...
                                        uint32_t sleep_time)
{
    ...
    if (0 != QCC74x_RD_REG(PDS_BASE, PDS_GPIO_I_SET)) {
        pPdsCfg->pdsCtl.ctrlGpioIePuPd = 1;
    }

    tmpVal = QCC74x_RD_REG(PDS_BASE, PDS_INT);
    /* if enable wakeup trigger by irrx */
    if (QCC74x_IS_REG_BIT_SET(tmpVal, PDS_WAKEUP_BY_IRRX_EN)) {
   ...
   * ldo11soc_rdy_aon */
    ...
    pds_ram[0] = QCC74x_RD_REG(PDS_BASE, PDS_RAM1);
    pds_ram[1] = QCC74x_RD_REG(PDS_BASE, PDS_RAM2);
    pds_ram[2] = QCC74x_RD_REG(PDS_BASE, PDS_RAM3);
    pds_ram[3] = QCC74x_RD_REG(PDS_BASE, PDS_RAM4);
...
/******************************* Wakeup Flow *******************************/
    ...
    if (pds_level < PM_PDS_LEVEL_2) {
        ...
        qcc74x_sflash_restore_from_powerdown(flash_cfg, 0, SF_CTRL_FLASH_BANK0);
    }
#endif
                                        ...
                                        uint64_t sleep_time)
{
    ...
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, HBN_LDO11_AON_VOUT_SEL, PM_HBN_LDO_LEVEL_DEFAULT);
    tmpVal = QCC74x_SET_REG_BITS_VAL(tmpVal, HBN_LDO11_RT_VOUT_SEL, PM_HBN_LDO_LEVEL_DEFAULT);
    ...
    switch (hbn_level) {
        case PM_HBN_LEVEL_0:
            ...
            tmpVal = QCC74x_CLR_REG_BIT(tmpVal, HBN_PWRDN_HBN_RTC);
            break;
        case PM_HBN_LEVEL_1:
        ...
        case PM_HBN_LEVEL_2:
            tmpVal = QCC74x_SET_REG_BIT(tmpVal, HBN_PWRDN_HBN_CORE);
            tmpVal = QCC74x_SET_REG_BIT(tmpVal, HBN_PWRDN_HBN_RTC);
            break;
    }
...
{
    QCC74x_WR_REG(HBN_BASE, HBN_RSV1, (uint32_t)wakeup_callback);
    /* Set HBN flag */
    QCC74x_WR_REG(HBN_BASE, HBN_RSV0, HBN_STATUS_ENTER_FLAG);
}
...
{
    QCC74x_WR_REG(HBN_BASE, HBN_RSV1, para);
    /* Set APP jump flag */
    QCC74x_WR_REG(HBN_BASE, HBN_RSV0, APP_JUMP_ENTER_FLAG);
}

void pm_hbn_out0_irq_register(void)
{
    qcc74x_irq_attach(HBN_OUT0_IRQn, (irq_callback)HBN_OUT0_IRQn, NULL);
    qcc74x_irq_enable(HBN_OUT0_IRQn);
}

void pm_hbn_out1_irq_register(void)
{
    qcc74x_irq_attach(HBN_OUT1_IRQn, (irq_callback)HBN_OUT1_IRQn, NULL);
...
void pm_rc32k_auto_cal(void)
{
    ...
    if(QCC74x_GET_REG_BITS_VAL(tmpVal, HBN_F32K_SEL)){
...
{
    qcc74x_irq_attach(PDS_WAKEUP_IRQn, (irq_callback)PDS_WAKEUP_IRQ, NULL);
