...
{
#ifdef romapi_qcc74x_gpio_init
    romapi_qcc74x_gpio_init(dev, pin, cfgset);
#else
    uint32_t cfg = 0;
    uint32_t reg_base;
...
{
#ifdef romapi_qcc74x_gpio_deinit
    romapi_qcc74x_gpio_deinit(dev, pin);
#else
#if defined(QCC743)
    qcc74x_gpio_pad_check(pin);
#endif
    qcc74x_gpio_init(dev, pin, GPIO_INPUT | GPIO_FLOAT);
#endif
}

void qcc74x_gpio_set(struct qcc74x_device_s *dev, uint8_t pin)
{
#ifdef romapi_qcc74x_gpio_set
    romapi_qcc74x_gpio_set(dev, pin);
#else
...
{
#ifdef romapi_qcc74x_gpio_reset
    romapi_qcc74x_gpio_reset(dev, pin);
#else
#if defined(QCC74x_undef) || defined(QCC74x_undef)
    uint32_t regval = getreg32(dev->reg_base + GLB_GPIO_CFGCTL32_OFFSET);
    putreg32(regval & ~(1 << (pin & 0x1f)), dev->reg_base + GLB_GPIO_CFGCTL32_OFFSET);
...
{
#ifdef romapi_qcc74x_gpio_int_init
    romapi_qcc74x_gpio_int_init(dev, pin, trig_mode);
#else
    uint32_t reg_base;
    uint32_t cfg_address;
    uint32_t regval;
    ...
    qcc74x_gpio_int_mask(dev, pin, true);
...
void qcc74x_gpio_int_mask(struct qcc74x_device_s *dev, uint8_t pin, bool mask)
{
#ifdef romapi_qcc74x_gpio_int_mask
    romapi_qcc74x_gpio_int_mask(dev, pin, mask);
#else
    uint32_t reg_base;
    uint32_t cfg_address;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_gpio_int_clear
    romapi_qcc74x_gpio_int_clear(dev, pin);
#else
    uint32_t reg_base;
    uint32_t cfg_address;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_gpio_uart_init
    romapi_qcc74x_gpio_uart_init(dev, pin, uart_func);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint8_t sig;
...
#endif
    qcc74x_gpio_init(dev, pin, (7 << GPIO_FUNC_SHIFT) | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_1);
#endif
}

int qcc74x_gpio_feature_control(struct qcc74x_device_s *dev, int cmd, size_t arg)
{
#ifdef romapi_qcc74x_gpio_feature_control
    return romapi_qcc74x_gpio_feature_control(dev, cmd, arg);
#else
    int ret = 0;
    uint32_t reg_base;
    uint32_t regval;
...
struct qcc74x_device_s *g_gpio_int = NULL;
...
void qcc74x_gpio_irq_attach(uint8_t pin, void (*callback)(uint8_t pin))
{
    ...
    qcc74x_gpio_int_mask(g_gpio_int, pin, true);
    g_gpio_irq_callback[pin].handler = callback;
    qcc74x_gpio_int_mask(g_gpio_int, pin, false);
    qcc74x_irq_attach(g_gpio_int->irq_num, gpio_all_isr, NULL);
}
