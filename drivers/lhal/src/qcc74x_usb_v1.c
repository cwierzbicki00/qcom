...
#define QCC74x_undef_USB_BASE ((uint32_t)0x4000D800)
#define QCC74x_undef_GLB_BASE ((uint32_t)0x40000000)
...
{
    uint32_t fifo_addr = QCC74x_undef_USB_BASE + USB_EP0_TX_FIFO_WDATA_OFFSET + ep_idx * 0x10;
...
static void qcc74x_usb_fifo2mem(uint8_t ep_idx, uint8_t *data, uint32_t length)
{
    uint32_t fifo_addr = QCC74x_undef_USB_BASE + USB_EP0_RX_FIFO_RDATA_OFFSET + ep_idx * 0x10;
...
static void qcc74x_usb_xcvr_config(void)
{
    uint32_t regval;

    regval = getreg32(QCC74x_undef_GLB_BASE + GLB_USB_XCVR_OFFSET);
    regval |= GLB_PU_USB;
    putreg32(regval, QCC74x_undef_GLB_BASE + GLB_USB_XCVR_OFFSET);
    ...
    putreg32(regval, QCC74x_undef_GLB_BASE + GLB_USB_XCVR_CONFIG_OFFSET);
...
static void qcc74x_usb_ep0_size(uint8_t ep_mps)
{
    uint32_t regval;

    regval = getreg32(QCC74x_undef_USB_BASE + USB_CONFIG_OFFSET);
    regval &= ~USB_CR_USB_EP0_SW_SIZE_MASK;
    regval |= (ep_mps << USB_CR_USB_EP0_SW_SIZE_SHIFT);
    putreg32(regval, QCC74x_undef_USB_BASE + USB_CONFIG_OFFSET);
}

static void qcc74x_usb_ep_config(uint8_t ep_idx, uint8_t ep_type, uint8_t dir, uint16_t ep_mps)
{
    uint32_t regval;

    regval = getreg32(QCC74x_undef_USB_BASE + USB_EP1_CONFIG_OFFSET + 4 * (ep_idx - 1));
    ...
    putreg32(regval, QCC74x_undef_USB_BASE + USB_EP1_CONFIG_OFFSET + 4 * (ep_idx - 1));
}

static void qcc74x_usb_ep_set_nak(uint8_t ep_idx)
{
    uint32_t regval;
...
static void qcc74x_usb_ep_set_ready(uint8_t ep_idx)
{
    uint32_t regval;
...
static uint16_t qcc74x_usb_get_rxcount(uint8_t ep_idx)
{
    return (getreg32(QCC74x_undef_USB_BASE + USB_EP0_FIFO_STATUS_OFFSET + 0x10 * ep_idx) & USB_EP0_RX_FIFO_CNT_MASK) >> USB_EP0_RX_FIFO_CNT_SHIFT;
}

// static uint16_t qcc74x_usb_get_txcount(uint8_t ep_idx)
// {
//     return (getreg32(QCC74x_undef_USB_BASE + USB_EP0_FIFO_STATUS_OFFSET + 0x10 * ep_idx) & USB_EP0_TX_FIFO_CNT_MASK) >> USB_EP0_TX_FIFO_CNT_SHIFT;
// }

static bool qcc74x_usb_ep_isbusy(uint8_t ep_idx)
{
    uint32_t regval;

    if (ep_idx == 0) {
        regval = (getreg32(QCC74x_undef_USB_BASE + USB_CONFIG_OFFSET) & USB_STS_USB_EP0_SW_RDY);
    } else {
        regval = (getreg32(QCC74x_undef_USB_BASE + USB_EP1_CONFIG_OFFSET + 4 * (ep_idx - 1)) & USB_STS_EP1_RDY);
    }
    return regval ? true : false;
}

static uint32_t qcc74x_usb_get_intstatus(void)
{
    uint32_t regval;

    regval = getreg32(QCC74x_undef_USB_BASE + USB_INT_STS_OFFSET);
    regval &= ~getreg32(QCC74x_undef_USB_BASE + USB_INT_MASK_OFFSET);
    regval &= getreg32(QCC74x_undef_USB_BASE + USB_INT_EN_OFFSET);
    return regval;
}

static void qcc74x_usb_int_clear(uint32_t int_clear)
{
    uint32_t regval;
    regval = getreg32(QCC74x_undef_USB_BASE + USB_INT_CLEAR_OFFSET);
    regval |= int_clear;
...
int usb_dc_init(uint8_t busid)
{
    ...
    qcc74x_gpio_init(gpio, GPIO_PIN_7, GPIO_ANALOG | GPIO_SMT_EN | GPIO_DRV_0);
    qcc74x_gpio_init(gpio, GPIO_PIN_8, GPIO_ANALOG | GPIO_SMT_EN | GPIO_DRV_0);

    qcc74x_usb_xcvr_config();
    ...
    putreg32(regval, QCC74x_undef_USB_BASE + USB_INT_EN_OFFSET);
...
int usbd_ep_start_write(uint8_t busid, const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
    ...
    if (ep_idx == 0) {
        if (data_len == 1) {
            qcc74x_usb_ep0_size(1);
        } else {
            qcc74x_usb_ep0_size(g_qcc74x_udc.in_ep[ep_idx].ep_mps);
        }
    }

    qcc74x_usb_mem2fifo(ep_idx, g_qcc74x_udc.in_ep[ep_idx].xfer_buf, data_len);
    qcc74x_usb_ep_set_ready(ep_idx);
...
void USBD_IRQHandler(uint8_t busid)
{
    ...
    intstatus = qcc74x_usb_get_intstatus();
    qcc74x_usb_int_clear(intstatus);
    ...
    for (uint8_t ep_idx = 1; ep_idx < USB_NUM_BIDIR_ENDPOINTS; ep_idx++) {
        if (intstatus & (1 << (9 + 2 * ep_idx))) {
            if (g_qcc74x_udc.in_ep[ep_idx].ep_enable) {
                ...
                g_qcc74x_udc.in_ep[ep_idx].xfer_buf += tx_count;
                g_qcc74x_udc.in_ep[ep_idx].xfer_len -= tx_count;
                ...
                while (qcc74x_usb_ep_isbusy(ep_idx)) {
                }

                rx_count = qcc74x_usb_get_rxcount(ep_idx);
                qcc74x_usb_fifo2mem(ep_idx, g_qcc74x_udc.out_ep[ep_idx].xfer_buf, rx_count);

                g_qcc74x_udc.out_ep[ep_idx].xfer_buf += rx_count;
                g_qcc74x_udc.out_ep[ep_idx].xfer_len -= rx_count;
                g_qcc74x_udc.out_ep[ep_idx].actual_xfer_len += rx_count;
    ...
    if (intstatus & USB_EP0_SETUP_DONE_INT) {
        while (qcc74x_usb_ep_isbusy(0)) {
        }

        rx_count = qcc74x_usb_get_rxcount(0);
        if (rx_count != 8) {
            printf("setup fail\r\n");
            return;
        }
    ...
    if (intstatus & USB_EP0_IN_DONE_INT) {
        ...
        g_qcc74x_udc.in_ep[0].xfer_buf += tx_count;
        ...
        } else if (g_qcc74x_udc.setup.wLength && ((g_qcc74x_udc.setup.bmRequestType & USB_REQUEST_DIR_MASK) == USB_REQUEST_DIR_OUT)) {
            /* In status, start reading setup */
            qcc74x_usb_ep_set_ready(0);
        }
    }
    if (intstatus & USB_EP0_OUT_DONE_INT) {
        ...
        qcc74x_usb_fifo2mem(0, g_qcc74x_udc.out_ep[0].xfer_buf, rx_count);
        g_qcc74x_udc.out_ep[0].xfer_buf += rx_count;
