...
void qcc74x_spi_init(struct qcc74x_device_s *dev, const struct qcc74x_spi_config_s *config)
{
    ...
    romapi_qcc74x_spi_init(dev, config);
#else
    uint32_t reg_base;
    uint32_t regval;
    uint32_t div;
    ...
    div = (qcc74x_clk_get_peripheral_clock(QCC74x_DEVICE_TYPE_SPI, dev->idx) / 2 * 10 / config->freq + 5) / 10;
    div = (div) ? (div - 1) : 0;
...
{
#ifdef romapi_qcc74x_spi_deinit
    romapi_qcc74x_spi_deinit(dev);
#else
    uint32_t regval;
    /* disable SPI */
...
{
#ifdef romapi_qcc74x_spi_link_txdma
    romapi_qcc74x_spi_link_txdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
{
#ifdef romapi_qcc74x_spi_link_rxdma
    romapi_qcc74x_spi_link_rxdma(dev, enable);
#else
    uint32_t reg_base;
    uint32_t regval;
...
ATTR_TCM_SECTION uint32_t qcc74x_spi_poll_send(struct qcc74x_device_s *dev, uint32_t data)
{
#ifdef romapi_qcc74x_spi_poll_send
    return romapi_qcc74x_spi_poll_send(dev, data);
#else
...
/* read and write data */
ATTR_TCM_SECTION int qcc74x_spi_poll_exchange(struct qcc74x_device_s *dev, const void *txbuffer, void *rxbuffer, size_t nbytes)
{
#ifdef romapi_qcc74x_spi_poll_exchange
    return romapi_qcc74x_spi_poll_exchange(dev, txbuffer, rxbuffer, nbytes);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
    uint32_t tx_cnt;
...
{
#ifdef romapi_qcc74x_spi_txint_mask
    romapi_qcc74x_spi_txint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
{
#ifdef romapi_qcc74x_spi_rxint_mask
    romapi_qcc74x_spi_rxint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
{
#ifdef romapi_qcc74x_spi_tcint_mask
    romapi_qcc74x_spi_tcint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
{
#ifdef romapi_qcc74x_spi_errint_mask
    romapi_qcc74x_spi_errint_mask(dev, mask);
#else
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;
...
{
#ifdef romapi_qcc74x_spi_get_intstatus
    return romapi_qcc74x_spi_get_intstatus(dev);
#else
    uint32_t reg_base;
    uint32_t int_status;
    uint32_t int_mask;
...
{
#ifdef romapi_qcc74x_spi_int_clear
    romapi_qcc74x_spi_int_clear(dev, int_clear);
#else
    uint32_t reg_base;
    uint32_t regval;
...
int qcc74x_spi_feature_control(struct qcc74x_device_s *dev, int cmd, size_t arg)
{
    ...
    switch (cmd) {
        ...
        case SPI_CMD_SET_FREQ:
            ...
            /* integer frequency segmentation by rounding */
            div = (qcc74x_clk_get_peripheral_clock(QCC74x_DEVICE_TYPE_SPI, dev->idx) / 2 * 10 / arg + 5) / 10;
            div = (div) ? (div - 1) : 0;
            div = (div > 0xff) ? 0xff : div;
        ...
        case SPI_CMD_GET_FREQ:
            ...
            ret = qcc74x_clk_get_peripheral_clock(QCC74x_DEVICE_TYPE_SPI, dev->idx) / div;
            break;
