...
static qcc74x_emac_irq_cb_t emac_irq_event_cb = NULL;
...
int qcc74x_emac_queue_tx_push(struct qcc74x_device_s *dev, struct qcc74x_emac_trans_desc_s *tx_desc)
{
    ...
    /* hw tx db address */
    uint32_t tx_db_addr = reg_base + EMAC_DMA_DESC_OFFSET + bd_index * sizeof(struct qcc74x_emac_hw_buff_desc_s);
...
int qcc74x_emac_queue_rx_push(struct qcc74x_device_s *dev, struct qcc74x_emac_trans_desc_s *rx_desc)
{
    ...
    /* hw rx db address */
    uint32_t rx_db_addr = reg_base + EMAC_DMA_DESC_OFFSET + (bd_index + 64) * sizeof(struct qcc74x_emac_hw_buff_desc_s);
