...
struct qcc74x_device_s *sim_dpi_dma_handle;

/* DMA linked list of SIM controls data */
static struct qcc74x_dma_channel_lli_pool_s dma_lli_dpi_header, dma_dpi_lli_tail;

/* DMA linked list of display buffer */
static struct qcc74x_dma_channel_lli_pool_s dma_lli_dpi_pool[LCD_DPI_SIM_DMA_LLI_CACHE_NUM][DPI_SIM_VALID_PIXEL_MAX / DMA_MAX_COUNT + 1];
...
int lcd_mipi_dpi_init(lcd_mipi_dpi_init_t *dpi_para)
{
    ...
    /* clk pin */
    qcc74x_gpio_init(gpio, LCD_DPI_SIM_PIN_CLK, GPIO_FUNC_SIM | GPIO_ALTERNATE | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_2);

    if (dpi_para->de_mode_en) {
        /* de pin */
        qcc74x_gpio_init(gpio, LCD_DPI_SIM_PIN_CLK + 1, GPIO_FUNC_SIM | GPIO_ALTERNATE | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_2);
    }

#if (LCD_SIM_DPI_DATA_LATCH_MODE)
    /* latch pin */
    qcc74x_gpio_init(gpio, LCD_DPI_SIM_PIN_CLK + 2, GPIO_FUNC_SIM | GPIO_ALTERNATE | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_2);
#endif

    /* hsync pin */
    qcc74x_gpio_init(gpio, LCD_DPI_SIM_PIN_HSYNC, GPIO_FUNC_SIM | GPIO_ALTERNATE | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_2);
    /* vsync pin */
    qcc74x_gpio_init(gpio, LCD_DPI_SIM_PIN_HSYNC + 1, GPIO_FUNC_SIM | GPIO_ALTERNATE | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_2);

#if (LCD_SIM_DPI_DATA_LATCH_MODE)
    /* data pin x8 */
    for (uint8_t i = 0; i < 8; i++) {
        qcc74x_gpio_init(gpio, LCD_DPI_SIM_PIN_DATA_START + i, GPIO_FUNC_SIM | GPIO_ALTERNATE | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_2);
    }
#else
    /* data pin x16 */
    ...
    /* sim init */
    struct qcc74x_sim_dpi_s dpi_sim;
    ...
    qcc74x_dma_channel_init(sim_dpi_dma_handle, &tx_config);

    /* attach interrupt */
    qcc74x_dma_channel_irq_attach(sim_dpi_dma_handle, sim_dpi_dma_interrupt, NULL);
    ...
    /* start dma */
    qcc74x_dma_channel_start(sim_dpi_dma_handle);
...
int lcd_mipi_dpi_screen_switch(void *screen_buffer)
{
    ...
    if (pix_format == LCD_MIPI_DPI_PIXEL_FORMAT_RGB565) {
        qcc74x_l1c_dcache_clean_range((void *)(screen_buffer), width * height * 2);
    } else if (pix_format == LCD_MIPI_DPI_PIXEL_FORMAT_NRGB888) {
        qcc74x_l1c_dcache_clean_range((void *)(screen_buffer), width * height * 4);
    }
...
static void sim_dpi_dma_interrupt(void *arg)
{
    ...
    /* stop dma channel */
    qcc74x_dma_channel_stop(sim_dpi_dma_handle);
...
static int sim_dpi_dma_lli_init(lcd_mipi_dpi_init_t *dpi_para)
{
    ...
    qcc74x_l1c_dcache_clean_range(sim_dpi_invalid_lin_para, sizeof(sim_dpi_invalid_lin_para));

    qcc74x_l1c_dcache_clean_range((void *)&dma_dpi_lli_tail, sizeof(dma_dpi_lli_tail));
    ...
    for (i = 0; i < LCD_DPI_SIM_DMA_LLI_CACHE_NUM; i++) {
        ...
        qcc74x_l1c_dcache_clean_range((void *)dma_lli_dpi_pool[i], sizeof(struct qcc74x_dma_channel_lli_pool_s) * (j + 1));
    }
...
static int sim_dpi_dma_lli_update(void *disp_buff)
{
    ...
    if (idx >= LCD_DPI_SIM_DMA_LLI_CACHE_NUM) {
        idx = idx_oldest;
        /* update dma lli */
        struct qcc74x_dma_channel_lli_pool_s *lli_p = dma_lli_cache_table[idx].dma_lli;
        for (uint16_t i = 0; i < sim_dpi_dma_lli_num; i++) {
            lli_p[i].src_addr = (uint32_t)(uintptr_t)(disp_buff) + (i * DMA_MAX_COUNT * 4);
        }
        qcc74x_l1c_dcache_clean_range((void *)lli_p, sizeof(struct qcc74x_dma_channel_lli_pool_s) * sim_dpi_dma_lli_num);
