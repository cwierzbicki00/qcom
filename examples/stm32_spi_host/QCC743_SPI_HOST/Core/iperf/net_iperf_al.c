...
no_copy_buf:
    if (net_iperf_nocopy_buf == NULL) {
        net_iperf_nocopy_buf = mem_malloc(IPERF_BUFFER_LEN + NET_IPERF_HEADER_TOTAL_SIZE);
        if (net_iperf_nocopy_buf == NULL) {
    ...
    {
        net_iperf_nocopy_buf[k + NET_IPERF_HEADER_TOTAL_SIZE] = 0x30 + k % 10;
    }
                                ...
                                struct fhost_iperf_settings* iperf_settings)
{
    ...
    while (write_more)
    {
        ...
        do
        {
            ...
            err = tcp_write(pcb, net_iperf_nocopy_buf + NET_IPERF_HEADER_TOTAL_SIZE, len, 0);
            if (err == ERR_MEM)
            {
                len /= 2;
            }
                                             ...
                                             int32_t packet_id)
{
    ...
    while ( cnt < 10 )
    {
        ...
        h = pbuf_alloced_custom(PBUF_TRANSPORT,
        		stream->iperf_settings.buf_len > NET_IPERF_HEADER_SIZE ? stream->iperf_settings.buf_len : NET_IPERF_HEADER_SIZE,
				PBUF_RAM, &(buf_tag->h), net_iperf_nocopy_buf, IPERF_BUFFER_LEN + NET_IPERF_HEADER_TOTAL_SIZE);
...
int net_iperf_tcp_client_run(struct fhost_iperf_stream* stream)
{
    ...
    // Start timer (http://jira.qcc74x.com/browse/QCC743-25)
