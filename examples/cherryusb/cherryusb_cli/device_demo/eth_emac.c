...
static void eth_emac_irq_cb(void *arg, uint32_t irq_event, struct qcc74x_emac_trans_desc_s *trans_desc);
...
/* emac */
static struct qcc74x_device_s *emac0;
/* eth phy */
...
static void eth_emac_irq_cb(void *arg, uint32_t irq_event, struct qcc74x_emac_trans_desc_s *trans_desc)
{
    ...
    switch (irq_event) {
        ...
        case EMAC_IRQ_EVENT_RX_CTRL_FRAME:
            ...
            qcc74x_emac_queue_rx_push(emac0, trans_desc);
            /* debug */
...
int eth_emac_init(void)
{
    ...
    qcc74x_emac_irq_attach(emac0, eth_emac_irq_cb, NULL);
    // qcc74x_emac_feature_control(emac0, EMAC_CMD_SET_RX_PROMISCUOUS, false);
...
int eth_emac_deinit(void)
{
    /* disable emac tx/rx */
    if (emac0) {
        qcc74x_irq_disable(emac0->irq_num);
...
bool eth_link_state_update(void)
{
    ...
    if (sta == EPHY_LINK_STA_UP && g_link_sta != EPHY_LINK_STA_UP) {
        ...
        if (speed == EPHY_SPEED_MODE_10M_HALF_DUPLEX || speed == EPHY_SPEED_MODE_10M_FULL_DUPLEX) {
            qcc74x_emac_feature_control(emac0, EMAC_CMD_SET_SPEED_10M, NULL);
        } else {
            qcc74x_emac_feature_control(emac0, EMAC_CMD_SET_SPEED_100M, NULL);
        }
#endif
        /* enable full duplex mode */
        if (speed == EPHY_SPEED_MODE_10M_FULL_DUPLEX || speed == EPHY_SPEED_MODE_100M_FULL_DUPLEX) {
            qcc74x_emac_feature_control(emac0, EMAC_CMD_SET_FULL_DUPLEX, true);
        } else {
...
int eth_emac_tx_buff_get(struct qcc74x_emac_trans_desc_s *trans_desc, uint32_t timeout)
{
...
int eth_emac_tx_buff_push(struct qcc74x_emac_trans_desc_s *trans_desc)
{
    trans_desc->attr_flag = 0;
    trans_desc->err_status = 0;
    ...
    int ret = qcc74x_emac_queue_tx_push(emac0, trans_desc);
    if (ret < 0) {
...
int eth_emac_rx_data_get(struct qcc74x_emac_trans_desc_s *trans_desc, uint32_t timeout)
{
...
int eth_emac_rx_data_free(struct qcc74x_emac_trans_desc_s *trans_desc)
{
    trans_desc->attr_flag = 0;
    trans_desc->err_status = 0;

    emac_debug_info.rx_push_cnt += 1;
    int ret = qcc74x_emac_queue_rx_push(emac0, trans_desc);
    if (ret < 0) {
...
{
    uint32_t tx_db_avail = qcc74x_emac_feature_control(emac0, EMAC_CMD_GET_TX_DB_AVAILABLE, 0);
    uint32_t rx_db_avail = qcc74x_emac_feature_control(emac0, EMAC_CMD_GET_RX_DB_AVAILABLE, 0);
