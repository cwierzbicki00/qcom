...
void dhcp_sta_update(struct netif *netif)
{
    ...
    switch (dhcp_sta) {
        ...
        case DHCP_TIMEOUT:
            ...
            /* retry */
            if (qcc74x_mtimer_get_time_ms() - time_ms > DHCP_RETRY_DELAY_MS) {
                dhcp_sta = DHCP_START;
            }
            break;
...
static err_t usbh_cdc_ecm_output(struct netif *netif, struct pbuf *p)
{
    ...
    if (g_ecm_out_busy_flag) {
        uint32_t time_start = qcc74x_mtimer_get_time_ms();
        while (g_ecm_out_busy_flag) {
            if ((qcc74x_mtimer_get_time_ms() - time_start) > 100) {
...
static void usbh_cdc_ecm_input_thread(CONFIG_USB_OSAL_THREAD_SET_ARGV)
{
    ...
    while (1) {
        ...
        if (g_ecm_int_busy_flag == false) {
            ...
            if (qcc74x_mtimer_get_time_ms() - int_time > 10) {
                int_time = qcc74x_mtimer_get_time_ms();
...
#if LWIP_DHCP
        /* dhcp update */
        if (qcc74x_mtimer_get_time_ms() - dhcp_time > 100) {
            dhcp_time = qcc74x_mtimer_get_time_ms();
