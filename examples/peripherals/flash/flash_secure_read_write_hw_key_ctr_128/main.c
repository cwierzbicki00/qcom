...
static void qcc74x_data_compare(const uint8_t *expected, uint8_t *input, uint32_t len)
{
    int i = 0;
    for (i = 0; i < len; i++) {
...
int main(void)
{
    ...
    qcc74x_aes_init(aes);
    qcc74x_aes_set_mode(aes, AES_MODE_CTR);
    qcc74x_aes_set_hwkey_source(aes, 0);
    qcc74x_aes_select_hwkey(aes,2,0);
    qcc74x_aes_setkey(aes, NULL, 128);

    qcc74x_aes_encrypt(aes, aes_ctr_pt_buffer, aes_ctr_128bit_iv, aes_ctr_ct_buffer, AES_DATA_LEN);
    qcc74x_data_compare(aes_ctr_ct_buffer, aes_ctr_ct_value, AES_DATA_LEN);
    printf("aes ctr128 encrypt success\r\n");

    qcc74x_flash_erase(data_store_addr, AES_DATA_LEN);
    memset(aes_ctr_tmp_buffer, 0, AES_DATA_LEN);

    qcc74x_flash_write(data_store_addr, aes_ctr_ct_buffer, AES_DATA_LEN);
    qcc74x_flash_read(data_store_addr, aes_ctr_tmp_buffer, AES_DATA_LEN);
    qcc74x_data_compare(aes_ctr_ct_buffer, aes_ctr_tmp_buffer, AES_DATA_LEN);
    printf("flash write & read success\r\n");

    qcc74x_aes_decrypt(aes, &aes_ctr_tmp_buffer[0], aes_ctr_128bit_iv, &aes_ctr_tmp_buffer2[0], AES_DATA_LEN);
    qcc74x_data_compare(aes_ctr_pt_buffer, aes_ctr_tmp_buffer2, AES_DATA_LEN);
