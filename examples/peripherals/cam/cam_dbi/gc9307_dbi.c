...
/* dma device */
static struct qcc74x_device_s *dbi_dma_hd;
/* pec dbi typeB device */
static struct qcc74x_device_s *dbi_hd;

/* The memory space of DMA */
static struct qcc74x_dma_channel_lli_pool_s dma_tx_llipool[LCD_DBI_DMA_LLI_NUM];
static struct qcc74x_dma_channel_lli_transfer_s dma_tx_transfers[1];
...
{
    /* disable dbi dma */
    qcc74x_dbi_link_txdma(dbi_hd, false);

    /* clear flag and mask int */
    qcc74x_dbi_int_clear(dbi_hd, DBI_INTCLR_TC);
    qcc74x_dbi_tcint_mask(dbi_hd, true);
    ...
    if (dbi_async_cycle_fill_flag == true) {
        ...
        qcc74x_dma_feature_control(dbi_dma_hd, DMA_CMD_SET_SRCADDR_INCREMENT, true);
    }
...
int lcd_dbi_init(lcd_dbi_init_t *dbi_parra)
{
    ...
    /* dbi cfg */
    struct qcc74x_dbi_config_s dbi_cfg = {
    ...
    /* dma cfg */
    struct qcc74x_dma_channel_config_s dma_dbi_config = {
    ...
    for (uint8_t i = 0; i < sizeof(dbi_gpio_list) / sizeof(dbi_gpio_list[0]); i++) {
        qcc74x_gpio_init(gpio, dbi_gpio_list[i], dbi_gpio_func | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_2);
    }
    qcc74x_gpio_init(gpio, LCD_DBI_TYPEB_PIN_CS, GPIO_OUTPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_1);
    qcc74x_gpio_reset(gpio, LCD_DBI_TYPEB_PIN_CS);
    qcc74x_gpio_init(gpio, LCD_DBI_TYPEB_PIN_RST, GPIO_OUTPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_1);
    qcc74x_gpio_reset(gpio, LCD_DBI_TYPEB_PIN_RST);
    qcc74x_mtimer_delay_ms(10);
    qcc74x_gpio_set(gpio, LCD_DBI_TYPEB_PIN_RST);

    /* dbi init */
    dbi_hd = qcc74x_device_get_by_name("dbi");
    qcc74x_dbi_init(dbi_hd, &dbi_cfg);
    /* cs continuous mode */
    qcc74x_dbi_feature_control(dbi_hd, DBI_CMD_CS_CONTINUE, true);

    /* dbi tx intertupt init */
    qcc74x_dbi_tcint_mask(dbi_hd, true);
    qcc74x_irq_attach(dbi_hd->irq_num, dbi_tc_callback, NULL);
    qcc74x_irq_enable(dbi_hd->irq_num);

    /* dma init */
    dbi_dma_hd = qcc74x_device_get_by_name(LCD_DBI_DMA_NAME);
    qcc74x_dma_channel_init(dbi_dma_hd, &dma_dbi_config);
...
{
    qcc74x_dbi_send_cmd_data(dbi_hd, cmd, para_num, (void *)para);
...
{
    qcc74x_dbi_send_cmd_pixel(dbi_hd, cmd, pixel_num, (void *)pixel);
...
int lcd_dbi_transmit_cmd_pixel_async(uint8_t cmd, uint32_t *pixel, size_t pixel_num)
{
    ...
    /* get data size */
    data_size = qcc74x_dbi_feature_control(dbi_hd, DBI_CMD_GET_SIZE_OF_PIXEL_CNT, pixel_num);
    ...
    qcc74x_dma_channel_lli_reload(dbi_dma_hd, dma_tx_llipool, LCD_DBI_DMA_LLI_NUM, dma_tx_transfers, 1);

    /* clean cache */
    qcc74x_l1c_dcache_clean_range((void *)pixel, data_size);

    /* trigger dbi data transfer */
    qcc74x_dbi_send_cmd_pixel(dbi_hd, cmd, pixel_num, NULL);

    /* unmask int */
    qcc74x_dbi_tcint_mask(dbi_hd, false);

    /* enabled DMA request for dbi */
    qcc74x_dbi_link_txdma(dbi_hd, true);

    /* enable dma */
    qcc74x_dma_channel_start(dbi_dma_hd);
...
int lcd_dbi_transmit_cmd_pixel_fill_sync(uint8_t cmd, uint32_t pixel_val, size_t pixel_num)
{
    ...
    /* get data size */
    words_cnt = qcc74x_dbi_feature_control(dbi_hd, DBI_CMD_GET_SIZE_OF_PIXEL_CNT, pixel_num) / 4;
    ...
    /* fill data into fifo */
    for (; words_cnt > 0;) {
        fifo_cnt = qcc74x_dbi_feature_control(dbi_hd, DBI_CMD_GET_TX_FIFO_CNT, 0);
    ...
    while ((qcc74x_dbi_get_intstatus(dbi_hd) & DBI_INTSTS_TC) == 0) {
    };
...
int lcd_dbi_transmit_cmd_pixel_fill_async(uint8_t cmd, uint32_t pixel_val, size_t pixel_num)
{
    ...
    /* dma cycle fill mode */
    qcc74x_dma_feature_control(dbi_dma_hd, DMA_CMD_SET_SRCADDR_INCREMENT, false);
    ...
    /* clean cache */
    qcc74x_l1c_dcache_clean_range((void *)&pixel_val, sizeof(pixel_val));
...
int gc9307_dbi_init()
{
    ...
    for (uint16_t i = 0; i < (sizeof(gc9307_dbi_init_cmds) / sizeof(gc9307_dbi_init_cmd_t)); i++) {
        if (gc9307_dbi_init_cmds[i].cmd == 0xFF && gc9307_dbi_init_cmds[i].data == NULL && gc9307_dbi_init_cmds[i].databytes) {
            qcc74x_mtimer_delay_ms(gc9307_dbi_init_cmds[i].databytes);
        } else {
