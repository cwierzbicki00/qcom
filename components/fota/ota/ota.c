...
static int ota_upgrade_slice(ota_handle_t handle, uint32_t offset, char *buf, uint32_t slice_size)
{
    ...
    for (retry = 0; retry < OTA_UPGRADE_RETRY; retry++) {
        ...
        ret = qcc74x_flash_write((handle->ota_addr + offset), buf, slice_size);
...
ota_handle_t ota_start(void)
{
    ...
    if (ota_handle->part_size < ota_handle->file_size) {
        LOG_E("[OTA] file_size overflow:0x%08x part_size:0x%08x\r\n", ota_handle->file_size, ota_handle->part_size);
        goto _fail;
    }
...
int ota_update(ota_handle_t handle, uint8_t *buf, uint32_t buf_len)
{
    ...
    if (!handle->head_checked) {
        if (handle->offset >= sizeof(ota_header_t)) {
            LOG_E("[OTA] header check fail %d %d\r\n", handle->offset);
            return -1;
        }
        ...
        if (_check_ota_header((ota_header_t *)handle->check_buf, &handle->file_size, &handle->use_xz)) {
    ...
    for (write_size = 0; write_size < buf_len; write_size += OTA_SLICE_SIZE) {
        ...
        ret = ota_upgrade_slice(handle, handle->offset + write_size, buf, slice_size);
        if (ret != 0) {
            return ret;
        }
