...
extern struct qcc74x_dma_channel_lli_pool_s txlli_pool[4];
extern struct qcc74x_dma_channel_lli_pool_s rxlli_pool[4];
uint32_t ATTR_NOCACHE_RAM_SECTION g_black_hole = 0xaabbccdd;//_attribute__((section(".wifi_ram"))) = 0xFFFFFFFF;

struct qcc74x_device_s *sgpio = NULL;
struct qcc74x_device_s *spi_dev = NULL;
struct qcc74x_device_s *dma0_ch0 = NULL;
struct qcc74x_device_s *dma0_ch1 = NULL;

int nx_dma_channel_lli_reload(struct qcc74x_device_s *dev, struct qcc74x_dma_channel_lli_pool_s *lli_pool, uint32_t max_lli_count, struct nx_dma_channel_lli_transfer_s *transfer, uint32_t count);
static int _hw_init(void (*callback)(void *arg), void *arg)
{
    ...
    //qcc74x_dma_channel_irq_attach(dma0_ch0, NULL, arg);
    qcc74x_dma_channel_irq_attach(dma0_ch1, callback, NULL);
}
...
{
    if (NULL == spi_dev) {
        spi_dev = qcc74x_device_get_by_name(NXSPI_NAME);
    }
    if (NULL == dma0_ch0) {
        dma0_ch0 = qcc74x_device_get_by_name(NXSPI_DMARXCH);
    }
    if (NULL == dma0_ch1) {
        dma0_ch1 = qcc74x_device_get_by_name(NXSPI_DMATXCH);
    }
    ...
    /* reset */
    qcc74x_dma_channel_stop(spi_dev);
    qcc74x_dma_channel_stop(spi_dev);
...
void nxspi_hwspi_transfer(uint8_t *send, uint8_t *recv, uint16_t len, void *cb, void *arg)
{
    ...
    if (NULL == send) {
        ...
        //qcc74x_dma_feature_control(dma0_ch0,  DMA_CMD_SET_SRCADDR_INCREMENT, 0);
        regval = *((volatile uint32_t *)0x2000C10C);//dma0
        regval &= ~(1<<26U);// src 0
        *((volatile uint32_t *)0x2000C10C) = regval;// dma0
    } else {
        //qcc74x_dma_feature_control(dma0_ch0,  DMA_CMD_SET_SRCADDR_INCREMENT, 1);
        regval = *((volatile uint32_t *)0x2000C10C);//dma0
        regval |= (1<<26U);// src 1
        *((volatile uint32_t *)0x2000C10C) = regval;// dma0
    }
    if (NULL == recv) {
        ...
        //qcc74x_dma_feature_control(dma0_ch1,  DMA_CMD_SET_DSTADDR_INCREMENT, 0);
        regval = *((volatile uint32_t *)0x2000C20C);//dma1
        regval &= ~(1<<27U);// dest 0
        *((volatile uint32_t *)0x2000C20C) = regval;// dma1
    } else{
        //qcc74x_dma_feature_control(dma0_ch1,  DMA_CMD_SET_DSTADDR_INCREMENT, 1);
        regval = *((volatile uint32_t *)0x2000C20C);//dma1
        regval |= (1<<27U);// dest 1
        *((volatile uint32_t *)0x2000C20C) = regval;// dma1
    }

    qcc74x_dma_channel_lli_reload(dma0_ch0, txlli_pool, DMATX_LLIPOOL_CNT, tx_transfers, 1);
    qcc74x_dma_channel_lli_reload(dma0_ch1, rxlli_pool, DMARX_LLIPOOL_CNT, rx_transfers, 1);
...
// todo: set reg
    if (NULL == sgpio) {
        sgpio = qcc74x_device_get_by_name("gpio");
    }
    if (0 == g_nxspi.gpio_inited) {
        /* spi clk */
        qcc74x_gpio_init(sgpio, NXSPI_GPIO_CLK, GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_FLOAT | GPIO_SMT_EN | GPIO_DRV_1);
        /* spi miso */
        qcc74x_gpio_init(sgpio, NXSPI_GPIO_MISO, GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_FLOAT | GPIO_SMT_EN | GPIO_DRV_1);
        /* spi mosi */
        qcc74x_gpio_init(sgpio, NXSPI_GPIO_MOSI, GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_FLOAT | GPIO_SMT_EN | GPIO_DRV_1);
    ...
    #else
        /* irq */
        qcc74x_gpio_init(sgpio, NXSPI_GPIO_IRQ, GPIO_OUTPUT | GPIO_INPUT | GPIO_PULLUP | GPIO_SMT_EN | GPIO_DRV_3);
    #endif
    ...
    /* cs */
    qcc74x_gpio_int_mask(sgpio, pin, true);
    qcc74x_irq_disable(sgpio->irq_num);

    qcc74x_gpio_int_init(sgpio, pin, trig_mode);//GPIO_INT_TRIG_MODE_SYNC_FALLING_RISING_EDGE);
    if (!attached) {
        qcc74x_gpio_irq_attach(pin, callback);
        attached = 1;
    }
    qcc74x_gpio_int_mask(sgpio, pin, false);
    qcc74x_irq_clear_pending(sgpio->irq_num);
    qcc74x_irq_enable(sgpio->irq_num);
}

void nxspi_hwgpio_disableirq(void)
{
    qcc74x_irq_disable(sgpio->irq_num);
    qcc74x_dma_channel_irq_detach(dma0_ch1);
}
...
{
    // todo: set reg
    return qcc74x_gpio_read(sgpio, pin);
}

void nx_dma_lli_config(struct qcc74x_device_s *dev,
...
uint8_t nx_gpio_trigmode_get(struct qcc74x_device_s *dev, uint8_t pin)
{
    uint32_t reg_base;
