...
extern struct qcc74x_device_s *sgpio;
...
struct qcc74x_dma_channel_lli_pool_s txlli_pool[4] __attribute__((section(".nocache_ram")));
struct qcc74x_dma_channel_lli_pool_s rxlli_pool[4] __attribute__((section(".nocache_ram")));
...
extern struct qcc74x_device_s *dma0_ch0;
extern struct qcc74x_device_s *dma0_ch1;
...
// cs isr
extern uint8_t nx_gpio_trigmode_get(struct qcc74x_device_s *dev, uint8_t pin);
...
void __trans_start()
{
    ...
    func_usetime_us = qcc74x_mtimer_get_time_us();
#endif
...
#if GPIO_TIME_ENABLE
    qcc74x_gpio_init(sgpio, NXSPI_GPIO_TIME, GPIO_OUTPUT | GPIO_INPUT | GPIO_PULLDOWN | GPIO_SMT_EN | GPIO_DRV_3);
