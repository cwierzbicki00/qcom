...
#if defined (QCC74x_BLE_GAP_SET_PERIPHERAL_PREF_PARAMS)
...
#if defined (QCC74x_BLE_ENABLE_OR_DISABLE_SLAVE_PREF_CONN_PARAM_UDPATE)
...
#endif//QCC74x_BLE_GAP_SET_PERIPHERAL_PREF_PARAMS
...
void bt_conn_process_tx(struct bt_conn *conn)
{
	...
	BT_DBG("conn %p", conn);
	#if defined(QCC74x_BLE_PATCH_NOT_DO_TX_IF_DISCONNECTED)
	if (conn->state == BT_CONN_DISCONNECTED){     
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	#if defined(QCC74x_BLE_PATCH_FIX_CONN_STATE_CHANGE_RISK)
	switch (state) {
	#else
	switch (conn->state) {
	#endif
	...
	case BT_CONN_DISCONNECTED:
		...
		if (old_state == BT_CONN_CONNECTED ||
			...
			if (conn->err) {
				notify_connected(conn);
				bt_conn_unref(conn);//qcc74x fix:unref the ref is increased in bt_conn_create_slave_le.
			}
	...
	case BT_CONN_CONNECT:
		...
		break;
	case BT_CONN_DISCONNECT:
		#if defined(QCC74x_BLE_PATCH_DISCONNECT_ERROR_WHEN_TASK_YEILD)
		if(old_state == BT_CONN_DISCONNECTED){
			conn->state = old_state;
			bt_conn_unref(conn);
		}
		#endif /* QCC74x_BLE_PATCH_DISCONNECT_ERROR_WHEN_TASK_YEILD */
		break;
	default:
					...
					const struct bt_le_adv_param *param)
{
    ...
    conn->role = BT_HCI_ROLE_SLAVE;//qcc74x fix
start_adv:
...
int bt_conn_init(void)
{
    ...
    #endif
#endif
#else //QCC74x_DYNAMIC_ALLOC_MEM
    struct net_buf_pool num_complete_pool;
    struct net_buf_pool acl_tx_pool;
#if CONFIG_BT_L2CAP_TX_FRAG_COUNT > 0
    struct net_buf_pool frag_pool;
#endif
#endif//QCC74x_DYNAMIC_ALLOC_MEM
