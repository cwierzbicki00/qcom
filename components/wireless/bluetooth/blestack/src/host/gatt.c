...
#endif
#if defined(QCC74x_BLE_DISCOVER_ONGOING)
...
#if defined(QCC74x_BLE_DISABLE_STATIC_ATTR)
...
static void sc_clear(struct gatt_sc_cfg *cfg)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		#if defined(QCC74x_BLE_PATCH_SC_CFG_FAIL_TO_BE_REMOVED_FROM_FLASH)
...
static void sc_indicate(u16_t start, u16_t end)
{
    ...
    #if defined (QCC74x_BLE_PATCH_SET_SCRANGE_CHAGD_ONLY_IN_CONNECTED_STATE)
...
#if defined(QCC74x_BLE_SET_LOCAL_ATT_MTU_SIZE)
...
static void remove_subscriptions(struct bt_conn *conn)
{
	...
	#if defined(QCC74x_BLE_AVOID_REMOVE_GATT_SUBSCRIPTION_RISK)
				   ...
				   void *user_data)
{
...
#if defined(QCC74x_BLE_AUTO_CANCEL_RELIABLE_WRITE_CHARACTERISTIC)
	...
	params->length -= len;
#endif /* QCC74x_BLE_AUTO_CANCEL_RELIABLE_WRITE_CHARACTERISTIC */
			       ...
			       void *user_data)
{
    ...
    #if defined(QCC74x_BLE_PATCH_NOTIFY_WRITE_CCC_RSP)
    else{
        params->notify(conn, params, NULL, 0);
    }
    #endif
...
#if defined(CONFIG_BT_GATT_CLIENT)
	#if !defined(QCC74x_BLE_NOT_FORCE_WRITE_CCC_FOR_EXISTED_SUBSCRIPTIONS_IN_STACK)
...
void bt_gatt_disconnected(struct bt_conn *conn)
{
	...
	#if defined(QCC74x_BLE_PATCH_CLEAR_UNBONDED_DEVICE_SC_WHEN_DISCONNECTED)
	if (!bt_addr_le_is_bonded(conn->id, &conn->le.dst)){
...
#endif
#if defined(QCC74x_BLE_DYNAMIC_SERVICE)
