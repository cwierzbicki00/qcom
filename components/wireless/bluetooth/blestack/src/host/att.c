	...
	while ((buf = net_buf_get(&att->tx_queue, K_NO_WAIT))) {
		...
		if (att->req && att->req->buf == buf) {
			...
			net_buf_simple_save(&att->req->buf->b,
					    &att->req->state);
			#if defined(QCC74x_BLE_PATCH_ATT_SEND_REQ_WHEN_TX_SEM_BUSY_BUF_ERR)
			if (!att_send(conn, net_buf_ref(buf), NULL, NULL)) {
				return;
			}
			net_buf_unref(buf);
			att->req->buf = NULL;
			#endif /* QCC74x_BLE_PATCH_ATT_SEND_REQ_WHEN_TX_SEM_BUSY_BUF_ERR */
			continue;
		}
...
static void att_process(struct bt_att *att)
{
	...
	BT_DBG("");
	#if defined(QCC74x_BLE_PATCH_AVOID_NEXT_ATT_REQ_SENT_BEFORE_PREVIOUS_ATT_RSP_RCVD)
...
{
    #if defined(QCC74x_BLE_DISABLE_STATIC_CHANNEL)
...
void bt_att_req_cancel(struct bt_conn *conn, struct bt_att_req *req)
{
	...
	#if defined(QCC74x_BLE_PATCH_CANCEL_ATT_TIMEOUT_TIMER_WHEN_ATT_REQ_CANCELLED)
	/* Cancel timeout if ongoing */
	k_delayed_work_cancel(&att->timeout_work);
	#endif
