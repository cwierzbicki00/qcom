...
extern QCC74x_Err_Type Sec_Eng_Trng_Enable(void);
int qcc74x_rand();
#else
extern int qcc74x_rand();
#endif
...
void *k_queue_get(struct k_queue *queue, s32_t timeout)
{
    ...
    if (timeout == K_FOREVER) {
        t = QCC74x_WAIT_FOREVER;
    } else if (timeout == K_NO_WAIT) {
        t = QCC74x_NO_WAIT;
    }

    ret = xQueueReceive(queue->hdl, &msg, t == QCC74x_WAIT_FOREVER ? portMAX_DELAY : ms2tick(t));
    if (ret == pdPASS) {
        return msg;
    } else {
        return NULL;
...
int k_sem_take(struct k_sem *sem, uint32_t timeout)
{
    ...
    ret = xSemaphoreTake(sem->sem.hdl, t == QCC74x_WAIT_FOREVER ? portMAX_DELAY : ms2tick(t));
