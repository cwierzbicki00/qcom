...
static int qcc74x_tp_send_indicate(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *data, u16_t len);
...
static void ble_tp_connected(struct bt_conn *conn, u8_t err)
{
    ...
    extern void QCC74x_undef_Delay_MS(uint32_t cnt);
    QCC74x_undef_Delay_MS(100);
    #else
...
/*************************************************************************
NAME    
    indicate_rsp /qcc74x_tp_send_indicate
*/ 
...
static void ble_tp_task(void *pvParameters)
{
    ...
    while(1){
         ...
         switch(*msg){
             ...
             {
                 char data[9] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09};
                 ret = qcc74x_tp_send_indicate(ble_tp_conn, get_attr(BT_CHAR_BLE_TP_IND_ATTR_VAL_INDEX), data, 9);
...
static int qcc74x_tp_send_indicate(struct bt_conn *conn, const struct bt_gatt_attr *attr,
