...
static void uart_isr(int irq, void *arg)
{
    ...
    if (intstatus & UART_INTSTS_RX_FIFO) {
        printf("rx fifo\r\n");
        while (qcc74x_uart_rxavailable(uartx) && !xStreamBufferIsFull(xStreamBufferRX)) {
          uint8_t data = qcc74x_uart_getchar(uartx);
...
static int32_t hci_uart_open(void) {
  ...
  uartx = qcc74x_device_get_by_name("uart1");
...
#if 0
  while(1) {
    qcc74x_uart_putchar(uartx, 0xa5);
    vTaskDelay(100);
  }
#endif
...
static int32_t hci_uart_write(uint8_t *buf, uint32_t nbytes) {
  ...
  while (nbytes - cur_n > wr_n && ret == 0) {
    ret += qcc74x_uart_put_block(uartx, buf + cur_n, wr_n);
    arch_delay_ms(50);
    cur_n += wr_n;
  }
  if (cur_n < nbytes) {
    ret += qcc74x_uart_put_block(uartx, buf + cur_n, nbytes - cur_n);
    cur_n = nbytes;
  }
  // printf("uw_ret=%d\n", ret);
  return nbytes;
  ...
  // printf("write%s\r\n", bt_hex(buf, nbytes));
  printf("uart_write\r\n");
  if (qcc74x_uart_put_block(uartx, buf, nbytes)) {
    return -1;
  } else {
    return nbytes;
  }
