...
#define LOG_INFO(fmt, ...)                                                     \
  printf("[%u] " fmt, qcc74x_timer_now_us(), ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...)                                                    \
  printf("[ERROR: %u] " fmt, qcc74x_timer_now_us(), ##__VA_ARGS__)
...
const static qcc74x_hci_trans_callbacks_t *hciTranCb;
...
static int uart_send(uint8_t *data, uint32_t len) {
  ...
  qcc74x_dma_channel_stop(dma_tx);
...
static int uart_send_ack(void) {
  ...
  crc32 = QCC74x_Soft_CRC32_Table(ack_data, sizeof(UARTCMD_HEAD));
...
static int uart_data_process(uint8_t *data, uint32_t len)
{
    ...
    crc32 = QCC74x_Soft_CRC32_Table(data, len - sizeof(crc32));
...
static int uart_find_data(uint8_t *data, int len, int *scan)
{
    ...
    if (head_pos >= 0) { // find valid head
        ...
        crc16 = QCC74x_Soft_CRC16(data + head_pos, sizeof(UARTCMD_HEAD) - 2);
...
static void UART_RTO_Callback(int irq, void *arg) {
  ...
  uint32_t len = qcc74x_dma_feature_control(dma_rx, DMA_CMD_GET_LLI_DSTADDR, 0) - (uint32_t)uart_rx_buf[uart_rx_buf_idx];
  //uint32_t crc32;
  ...
  qcc74x_dma_channel_stop(dma_rx);
  qcc74x_dma_channel_lli_reload(dma_rx, rx_llipool, sizeof(rx_llipool)/sizeof(rx_llipool[0]), &transfer, 1);
  qcc74x_dma_channel_start(dma_rx);

#if SHOW_RX_DATA != 0
  LOG("[%u] uart rx %" PRIu32 ":", qcc74x_timer_now_us(), len);
...
static void gpio_init(void) {
    ...
    qcc74x_gpio_uart_init(gpio, uart_tx_pin, GPIO_UART_FUNC_UART0_TX + 4 * uart->idx);
    qcc74x_gpio_uart_init(gpio, uart_rx_pin, GPIO_UART_FUNC_UART0_RX + 4 * uart->idx);
}
...
static void uart_init(void) {
    ...
    qcc74x_irq_attach(uart->irq_num, UART_RTO_Callback, NULL);
...
static void dma_rx_init(void) {
    ...
    qcc74x_dma_channel_irq_attach(dma_rx, UART_RX_DMA_Callback, NULL);
}
...
static void dma_tx_init(void) {
    ...
    qcc74x_dma_channel_irq_attach(dma_tx, UART_TX_DMA_Callback, NULL);
}
...
#if SHOW_TX_DATA != 0
  LOG("[%u] uart tx %" PRIu32 ":", qcc74x_timer_now_us(), len);
  for (uint32_t i = 0; i < len; i++) {
    LOG(" %02x", data[i]);
  }
  LOG("\r\n");
...
static void hci_uart_ack_task(void *pvParameters) {
  ...
  while(1) {
    if (xSemaphoreTake(uart_ack_sem, portMAX_DELAY) == pdTRUE) {
      ...
      crc16 = QCC74x_Soft_CRC16(ack_data, sizeof(UARTCMD_HEAD) - 2); // calc head crc16
...
int hci_uart_write(uint8_t *data, uint32_t len) {
  ...
  crc16 = QCC74x_Soft_CRC16(msg.data, sizeof(UARTCMD_HEAD) - 2); // calc head crc16
  memcpy(msg.data + sizeof(UARTCMD_HEAD) - 2, &crc16, sizeof(crc16));
  memcpy(msg.data + sizeof(UARTCMD_HEAD), data, len);
  crc32 = QCC74x_Soft_CRC32_Table(msg.data, sizeof(UARTCMD_HEAD) + len);
...
int hci_uart_is_busy(void) {
  ...
  // rx busy
  if (qcc74x_dma_feature_control(dma_rx, DMA_CMD_GET_LLI_DSTADDR, 0) != (uint32_t)uart_rx_buf[uart_rx_buf_idx]) {
    return 2;
  }
  ...
  if ((xTaskGetTickCountFromISR() - hci_wake_tick) < pdMS_TO_TICKS(2*UART_ACK_TIMEOUT_MS)) {
    ...
    qcc74x_pm_event_bit_clear(PSM_EVENT_BLE_HCI);
  }
  ...
  do {
    ...
    case UART_GET_BUFFER:
      ...
      case IND_ACL_DATA:
        ...
        // uart_ctx.rxBuf = (struct net_buf*)qcc74x_hci_alloc_buf(IND_ACL_DATA, 0,
        // len);
      ...
      case IND_EVENT:
        ...
        // uart_ctx.rxBuf = (struct net_buf*)qcc74x_hci_alloc_buf(IND_EVENT,
    ...
    case UART_GET_DATA:
      if (uart_ctx.buffType == IND_EVENT) {
        ...
        // qcc74x_hci_recv(uart_ctx.rxBuf);
      } else if (uart_ctx.buffType == IND_ACL_DATA) {
        hciTranCb->rx_handler(uart_ctx.rxBuf);
        // qcc74x_hci_recv(uart_ctx.rxBuf);
      }
...
static int32_t qcc74x_uart_close(void) {
  int ret = 0;
...
struct mdev_t *qcc74x_hci_uart_init(const qcc74x_hci_trans_callbacks_t *cb) {
  uart_dev.name = "uart";
  uart_dev.open = hci_uart_open;
  uart_dev.close = qcc74x_uart_close;
  uart_dev.read = NULL;
