...
void qcc74x_hci_recv(struct net_buf *rx_buf);
struct net_buf *qcc74x_hci_alloc_buf(uint8_t rx_type, uint8_t sub_evt,

                                 uint16_t len);

qcc74x_hci_trans_callbacks_t hci_callbacks = {
    qcc74x_hci_recv,
    qcc74x_hci_alloc_buf,
};

int qcc74x_hci_init(char *port) {
  hci_dev = qcc74x_hci_uart_init(&hci_callbacks);
  if (!hci_dev)
    return -ENODEV;
...
__attribute__((weak)) int qcc74x_hci_reset(void)
{
  return 0;
}
