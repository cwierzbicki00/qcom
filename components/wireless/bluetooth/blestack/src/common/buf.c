...
#endif
{
	...
	while (buf) {
		...
		unsigned int key = irq_lock();/* Added by qcc74x lab, to protect ref decrease */
		if (--buf->ref > 0) {
			irq_unlock(key);/* Added by qcc74x lab */
			return;
		}
		irq_unlock(key);/* Added by qcc74x lab */
...
struct net_buf *net_buf_ref(struct net_buf *buf)
{
	...
	unsigned int key = irq_lock();/* Added by qcc74x lab,  to protect ref increase */
	buf->ref++;
	irq_unlock(key);/* Added by qcc74x lab */
	return buf;
}
