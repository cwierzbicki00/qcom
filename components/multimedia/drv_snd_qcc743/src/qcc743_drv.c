...
static snd_qcc743_config_t *g_audio_gain_config = NULL;
...
//#define DMA_NUM                 (0)
/* input fixed config for qcc743 ，other configurations are not supported for now */
...
static msp_dev_t *card_init(msp_driver_t *drv, void *config, int id)
{
    ...
    xcodec_output_t *codec = msp_zalloc(sizeof(xcodec_output_t));
    //codec->pa_pin = ((snd_qcc743_config_t*)config)->pa_pin;
...
void snd_card_qcc743_register(void *config)
{
    if (config) {
...
void snd_card_qcc743_unregister(void *config)
{
