...
static int _codec_auio_test(uint32_t timeout)
{
    ...
    time_end = timeout + qcc74x_os_clock_gettime_ms(); ///< playback 60S
...
#endif
        if (qcc74x_os_clock_gettime_ms() > time_end) {
            break;
        }
        qcc74x_os_msleep(1);
    }
...
{
    int ret;
    g_hw_outbuf = qcc74x_os_malloc(HW_OUTBUF_SIZE);
    g_hw_input = qcc74x_os_malloc(HW_INBUF_SIZE);
