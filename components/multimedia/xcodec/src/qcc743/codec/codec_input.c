...
static int _aui_set_buffer(aui_ch_t *context, uint32_t initial, uint32_t threshold)
{
    ...
    for (i = initial; i < threshold; i++) {
        ...
        if (context->dma->node[i].dirty == 1) {// 0-dirty; 1-clear; 2-valid data
            ...
            if (context->callback) {
                context->callback(context, QCC74x_EVENT_NODE_READ_COMPLETE, context->arg);
            }
        }
    }
...
static void _aui_set_used_buffer(aui_ch_t *context)
{
    ...
    if (res) {
        ...
        context->callback(context, QCC74x_EVENT_READ_BUFFER_FULL, context->arg);
    }
}
#endif
...
static int _aui_rx_dma_link(aui_ch_t *context, void *dma)
{
    ...
    device_dma = qcc74x_device_get_by_dma_name("device_dma");
    ...
    qcc74x_dma_channel_irq_attach(device_dma, _aui_rx_dma_irq_hander, (void *)(context));
