...
static int _auo_clean_buffer(auo_ch_t *context, uint32_t initial, uint32_t threshold)
{
    ...
    for (i = initial; i < threshold; i++) {
        ...
        if (context->dma->node[i].dirty == 2) {// 0-dirty; 1-clear; 2-valid data
            ...
            if (context->callback) {
                context->callback(context, QCC74x_EVENT_NODE_WRITE_COMPLETE, context->arg);
            }
        }
...
static void _auo_clean_used_buffer(auo_ch_t *context)
{
    ...
    if (res) {
        context->callback(context, QCC74x_EVENT_WRITE_BUFFER_EMPTY, context->arg);
...
static int _auo_tx_dma_link_destroy(auo_ch_t *context, void *dma)
{
    ...
    qcc74x_dma_channel_irq_detach(context->device_dma);
    qcc74x_dma_channel_stop(context->device_dma);
...
static int _auo_tx_dma_link(auo_ch_t *context, void *dma)
{
    ...
    dma_ch = context->ch_id;
#else
    lli = (struct qcc74x_dma_channel_lli_pool_s *)hw_node_addr;
    ...
    struct qcc74x_device_s *device_dma;
    device_dma = qcc74x_device_get_by_name("device_dma");
    ...
    qcc74x_dma_channel_init(device_dma, &config);

    qcc74x_dma_channel_irq_attach(device_dma, _auo_tx_dma_irq_hander, (void *)(context));

    struct qcc74x_dma_channel_lli_transfer_s *transfers;

    transfers = (struct qcc74x_dma_channel_lli_transfer_s *)msp_malloc(sizeof(struct qcc74x_dma_channel_lli_transfer_s) * dma_node_num);
    ...
    qcc74x_dma_channel_lli_reload(device_dma, lli, dma_node_num, transfers, dma_node_num);
    qcc74x_dma_channel_start(device_dma);
#endif
#if 1
    dma_id = context->ctrl_id;
    dma_ch = context->ch_id;
    ...
    device_dma = qcc74x_device_get_by_name(dma_name);  
    ...
    qcc74x_dma_channel_init(device_dma, &tx_config);
...
// Pause DMA transfer and clear the data in the buffer.
...
int auo_stop(auo_ch_t *context)
{
    ...
    if (QCC74x_AUDIO_LEFT_CHANNEL == context->sound_channel_num) {
        Audio_TxFifoDisable(QCC74x_AUDIO_LEFT_CHANNEL_);
    } else if (QCC74x_AUDIO_STEREO_CHANNEL == context->sound_channel_num) {
        Audio_TxFifoDisable(QCC74x_AUDIO_STEREO_CHANNEL_);
    } else {
    }
#endif
